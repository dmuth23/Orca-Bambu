...
std::string run_and_capture(const std::wstring& exe_path, const std::vector<std::wstring>& args, DWORD* exit_code = nullptr)
{
    ...
    if (!::CreatePipe(&read_pipe, &write_pipe, &sa, 0)) {
        if (exit_code) *exit_code = static_cast<DWORD>(-1);
        return {};
    }
    ...
    const BOOL ok = ::CreateProcessW(exe_path.c_str(), mutable_command.data(),
    ...
    if (exit_code) *exit_code = process_exit;
...
std::string wsl_exe_path()
{
    ...
    if (size == 0 || size >= path.size()) return "wsl.exe";
    path.resize(size);
...
bool probe_wsl_ready(const std::string& distro, std::string* reason)
{
    ...
    if (!std::filesystem::exists(std::filesystem::u8path(wsl))) {
        if (reason) *reason = "wsl.exe not found in Windows system directory";
        return false;
    }
    ...
    if (status_code != 0) {
        if (reason) *reason = trim_ascii(status_out.empty() ? "wsl --status failed" : status_out);
        return false;
    }
    if (distro.empty()) {
    ...
    DWORD probe_code = 0;
    const std::string probe_out = run_and_capture(wsl_w,
        {L"-d", widen(distro), L"--user", L"root", L"--", L"sh", L"-lc", L"true"}, &probe_code);
    if (probe_code == 0) {
        if (reason) reason->clear();
        return true;
    }
    ...
    if (lowered.find("there is no distribution with the supplied name") != std::string::npos ||
        ...
        if (reason) *reason = "WSL distro '" + distro + "' is not installed";
        return false;
    }
    if (reason) *reason = message.empty() ? ("Failed to start WSL distro '" + distro + "'") : message;
    return false;
}
...
bool ensure_runtime_interactive(const std::filesystem::path& plugin_dir, const std::string& distro)
{
    ...
    std::string reason;
    if (probe_wsl_ready(distro, &reason)) return true;
    ...
    if (exit_code == 0) return probe_wsl_ready(distro, nullptr);
    ...
    if (exit_code == 1641 || exit_code == 3010) {
        ::MessageBoxW(nullptr,
            ...
            title.c_str(), MB_ICONWARNING | MB_OK | MB_SYSTEMMODAL);
        return false;
    }
